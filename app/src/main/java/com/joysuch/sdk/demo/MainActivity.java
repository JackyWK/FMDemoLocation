...
public class MainActivity extends Activity implements OnClickListener {
    ...
    FMMapExtent ex;                       //地图范围
    FMLocationLayer locLayer;             //定位图层
	ArrayList<FMGroupInfo> groups;
    Button btLoc;                         //定位图标
	int focus = 0;                //焦点层

	FMImageLayer mlForNaviStart;    //标注物图层，起点
	FMImageLayer mlForNaviEnd;      //标注物图层，终点
	FMLineLayer lineLayer;                //线图层
	FMNaviAnalyser naviAnalyser;
	FMMapCoord startPt,endPt;			  //起点,终点
	FMMapCoord picPt,locPt,analyserPt;          //点击点，定位点，查询分析点
	FMSearchAnalyser searchAnalyser;        //搜索分析器
	int startGroupId,endGroupId;          //起点，终点所在组Id
	int picGroupId,locGroupId,analyserId;            //点击点，定位点，查询分析点所在Id
	int[] gids;

	Button btAnalyser;                      //查询分析按钮
	EditText etKw;                         //搜索框
...
//		FMMapDataManager mapDataManager = FMMapDataManager.getDataManager();
//		mapDataManager.setDirectory(Environment.getExternalStorageDirectory() + "/");
...
//		map.openMapByPath(Environment.getExternalStorageDirectory() + "/90254.fmap");  //打开地图-离线
